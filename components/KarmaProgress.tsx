...
export default function KarmaProgress() {
  ...
  const nextLevelName =
    permissions.level < 5
      ? `${t("gamification.level")} ${permissions.level + 1}`
      : t("gamification.maxLevel");

  return (
    <div className="card space-y-4 sm:space-y-6 p-6 sm:p-8">
      {/* Header */}
          ...
          <div className="space-y-2">
            ...
            >
              <div
                ...
                style={{
                  ...
                  minWidth:
                    permissions.progressToNextLevel > 10 ? "auto" : "40px",
                }}
              >
            ...
            >
              <span className="whitespace-nowrap">
                {formatKarma(permissions.karma)} actual
              </span>
              <span className="font-semibold whitespace-nowrap">
                {permissions.karmaToNextLevel}{" "}
                {t("gamification.karmaRemaining")}
              </span>
              <span className="hidden sm:inline whitespace-nowrap">
                {formatKarma(permissions.karma + permissions.karmaToNextLevel)}{" "}
                necesario
              </span>
            </div>
          </div>
      ...
      >
        <h4
          className="font-semibold mb-2 text-sm sm:text-base flex items-center gap-2"
          style={{ color: "var(--brand)" }}
        >
          💡 {t("gamification.howToEarn")}
        </h4>
        <ul
          className="text-xs sm:text-sm space-y-1.5"
          style={{ color: "var(--foreground)" }}
        >
          <li className="flex items-start gap-2">
            <span className="text-blue-500 font-bold">•</span>
            <span>
              {t("gamification.earnThreads")}{" "}
              <strong className="text-blue-500">
                ({t("gamification.earnThreadsDetail")})
              </strong>
            </span>
          </li>
          <li className="flex items-start gap-2">
            ...
            <span>
              {t("gamification.earnComments")}{" "}
              <strong className="text-green-500">
                ({t("gamification.earnCommentsDetail")})
              </strong>
            </span>
          </li>
          <li className="flex items-start gap-2">
            ...
            <span>
              {t("gamification.earnLikes")}{" "}
              <strong className="text-orange-500">
                ({t("gamification.earnLikesDetail")})
              </strong>
            </span>
          </li>
          {permissions.level >= 2 && (
            <li className="flex items-start gap-2">
              ...
              <span>
                Marca threads útiles como recursos{" "}
                <strong className="text-cyan-500">(+10 karma)</strong>
              </span>
            </li>
          )}
          {permissions.level >= 3 && (
            <li className="flex items-start gap-2">
              ...
              <span>
                Da superlikes a contenido excepcional{" "}
                <strong className="text-purple-500">(+2 karma)</strong>
              </span>
            </li>
          )}
        </ul>
      ...
      {permissions.level < 5 && (
        ...
        >
          ...
          >
            ...
            {t("profile.karma").toLowerCase()} para desbloquear{" "}
            <strong>
              {permissions.levelName.replace("Nivel", t("gamification.level"))}{" "}
              {permissions.level + 1}
            </strong>
          </p>
        </div>
